...
const form = useForm({
    ...
    starts_at:             props.coupon.starts_at_input ?? '',
    expires_at:            props.coupon.expires_at_input ?? '',
...
function submit() {
    form.transform(data => ({ ...data, _method: 'PUT' }))
        .post(route('admin.coupons.update', props.coupon.id))
}

function destroy() {
    if (!confirm(`Supprimer le code promo ${props.coupon.code} ? Cette action est irréversible.`)) return
    useForm({}).delete(route('admin.coupons.destroy', props.coupon.id))
}
</script>
    ...
    <div class="p-6 space-y-5">
        ...
        <div class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-bold text-gray-900">Modifier le code promo</h1>
                    <p class="text-[13px] text-gray-500 mt-0.5 font-mono">{{ coupon.code }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid lg:grid-cols-3 gap-5">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="bg-white rounded-xl border border-red-200 p-5 space-y-3">
                    <h2 class="text-sm font-semibold text-red-700">Zone dangereuse</h2>
                    <p class="text-[13px] text-gray-500">
                        Si ce code a déjà été utilisé, il sera désactivé à la place d'être supprimé.
                    </p>
