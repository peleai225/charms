...
<form method="POST" action="{{ route('admin.coupons.update', $coupon) }}" class="space-y-6" id="couponForm">
    ...
    <div class="flex items-center justify-between">
        ...
        <button type="submit" class="px-6 py-2 bg-blue-600 text-white font-medium rounded-xl hover:bg-blue-700">Mettre à jour</button>
    </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="flex gap-2">
                                <input type="text" name="code" value="{{ old('code', $coupon->code) }}" class="flex-1 px-4 py-2 border border-slate-300 rounded-xl uppercase font-mono" required>
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $coupon->name) }}" class="w-full px-4 py-2 border border-slate-300 rounded-xl" required>
                            @error('name')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Description</label>
                        <textarea name="description" rows="2" class="w-full px-4 py-2 border border-slate-300 rounded-xl">{{ old('description', $coupon->description) }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-4">
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="type" id="couponType" class="w-full px-4 py-2 border border-slate-300 rounded-xl" onchange="toggleValueField()" required>
                                ...
                                <option value="fixed" {{ old('type', $coupon->type) === 'fixed' ? 'selected' : '' }}>Montant fixe (F CFA)</option>
                        ...
                        <div id="valueField">
                            ...
                            <input type="number" name="value" value="{{ old('value', $coupon->value) }}" step="0.01" min="0" class="w-full px-4 py-2 border border-slate-300 rounded-xl">
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="min_order_amount" value="{{ old('min_order_amount', $coupon->min_order_amount) }}" step="100" min="0" class="w-full px-4 py-2 border border-slate-300 rounded-xl" placeholder="Ex: 10000">
                        ...
                        <div>
                            ...
                            <input type="number" name="max_discount_amount" value="{{ old('max_discount_amount', $coupon->max_discount_amount) }}" step="100" min="0" class="w-full px-4 py-2 border border-slate-300 rounded-xl" placeholder="Ex: 5000">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="usage_limit" value="{{ old('usage_limit', $coupon->usage_limit) }}" min="1" class="w-full px-4 py-2 border border-slate-300 rounded-xl" placeholder="Illimité">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Limite par client</label>
                        <input type="number" name="usage_limit_per_user" value="{{ old('usage_limit_per_user', $coupon->usage_limit_per_user) }}" min="1" class="w-full px-4 py-2 border border-slate-300 rounded-xl" placeholder="Illimité">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="date" name="starts_at" value="{{ old('starts_at', $coupon->starts_at?->format('Y-m-d')) }}" class="w-full px-4 py-2 border border-slate-300 rounded-xl">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Date de fin</label>
                        <input type="date" name="expires_at" value="{{ old('expires_at', $coupon->expires_at?->format('Y-m-d')) }}" class="w-full px-4 py-2 border border-slate-300 rounded-xl">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <div class="space-y-4">
                    <label class="flex items-center gap-3">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $coupon->is_active) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 rounded">
                        <span class="text-slate-700">Actif</span>
                    </label>
                    <label class="flex items-center gap-3">
                        <input type="checkbox" name="first_order_only" value="1" {{ old('first_order_only', $coupon->first_order_only) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 rounded">
