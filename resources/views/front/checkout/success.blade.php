...
<div class="min-h-screen bg-gradient-to-b from-emerald-50/60 via-white to-white">
    ...
    <div class="container mx-auto px-4 py-10 max-w-3xl">
        ...
        @php
            $waItems = $order->items->map(fn($i) => "• {$i->name} x{$i->quantity}")->join("\n");
            $waMessage = "✅ Confirmation commande #{$order->order_number}\n\n"
                . "{$waItems}\n\n"
                . "Total : " . number_format($order->total, 0, ',', ' ') . " F CFA\n"
                . "Livraison : {$order->shipping_address}, {$order->shipping_city}\n\n"
                . "Suivi : " . route('order-tracking.index');
            $waPhone = preg_replace('/[^0-9]/', '', $order->shipping_phone ?? '');
        ...
        <div class="bg-green-50 border border-green-200 rounded-2xl p-5 mb-6 text-center">
            <p class="text-sm text-green-800 mb-3">Sauvegardez les détails de votre commande sur WhatsApp :</p>
            <a href="{{ $waUrl }}" target="_blank"
               class="inline-flex items-center gap-2 px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition-colors">
