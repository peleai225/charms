...
<body class="bg-slate-50 text-slate-900 font-sans antialiased min-h-screen overflow-x-hidden" 
    ...
    {{-- Bannière superadmin (violet, distincte de la barre admin) --}}
    @if($__authUser && $__authUser->role === 'superadmin')
    <div id="superadmin-bar" style="height:36px;padding-top:0" class="fixed top-0 inset-x-0 z-[400] bg-violet-700 text-white text-xs flex items-center">
        <div class="max-w-7xl mx-auto px-3 w-full flex items-center justify-between gap-3">
            <div class="flex items-center gap-2 min-w-0">
                ...
                <span class="font-semibold text-white truncate">{{ $__authUser->name }}</span>
            </div>
            <div class="flex items-center gap-1">
                ...
                <button onclick="document.getElementById('superadmin-bar').remove();document.body.style.paddingTop=''"
    ...
    {{-- Barre admin rapide (admin / manager / staff) --}}
    @elseif($__authUser && in_array($__authUser->role, ['admin', 'manager', 'staff']))
    <div id="admin-bar" style="height:36px" class="fixed top-0 inset-x-0 z-[300] bg-slate-900 text-white text-xs flex items-center">
        <div class="max-w-7xl mx-auto px-3 w-full flex items-center justify-between gap-3">
            <div class="flex items-center gap-2 min-w-0">
                ...
                <span class="font-semibold text-indigo-300">{{ $__authUser->name }}</span>
                <span class="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase
                    @if($__authUser->role === 'admin') bg-red-500/20 text-red-300
                    @elseif($__authUser->role === 'manager') bg-amber-500/20 text-amber-300
            ...
            <div class="flex items-center gap-1 overflow-x-auto scrollbar-none">
                ...
                @if(in_array($__authUser->role, ['admin', 'manager']))
    ...
    <div x-data="pwaInstall()" x-show="showBanner" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="translate-y-full opacity-0" x-transition:enter-end="translate-y-0 opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="translate-y-0 opacity-100" x-transition:leave-end="translate-y-full opacity-0" x-cloak class="fixed bottom-0 inset-x-0 z-[200]" style="padding-bottom: env(safe-area-inset-bottom, 0px);">
        <div class="mx-auto max-w-lg px-4 pb-4">
            <div class="bg-white rounded-2xl shadow-2xl shadow-slate-900/20 border border-slate-200 p-4 sm:p-5">
                {{-- Android natif — Chrome a le prompt, bouton "Installer" --}}
                <template x-if="platform === 'android-native'">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-2 mt-3">
                                <button @click="installApp()" class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-xs font-semibold rounded-lg shadow-sm transition-colors duration-150">Installer</button>
                                <button @click="dismiss()" class="px-4 py-2 text-xs font-medium text-slate-500 hover:text-slate-700 transition-colors">Plus tard</button>
                            </div>
                        </div>
                ...
                {{-- Android guide — pas encore de prompt natif, instructions manuelles --}}
                <template x-if="platform === 'android-guide'">
                    <div class="flex items-start gap-4">
                        <div class="flex-shrink-0 w-12 h-12 bg-primary-600 rounded-lg flex items-center justify-center shadow-sm">
                            <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                        </div>
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-slate-500 mt-1 leading-relaxed">
                                Appuyez sur <strong>⋮</strong> en haut à droite de Chrome, puis <strong>"Ajouter à l'écran d'accueil"</strong>
                            </p>
                            <div class="mt-3">
                                <button @click="dismiss()" class="px-4 py-2 text-xs font-medium text-slate-500 hover:text-slate-700 transition-colors">J'ai compris</button>
                            </div>
                        </div>
                        <button @click="dismiss()" class="flex-shrink-0 p-1 text-slate-400 hover:text-slate-600 transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
    ...
    <script>
        ...
        function pwaInstall() {
            return {
                showBanner: false,
                platform: 'android',   // 'android-native' | 'android-guide' | 'ios'
                ...
                init() {
                    ...
                    // Android/Chrome — deux cas :
                    // 1. Chrome a déjà validé le site → beforeinstallprompt disponible → bouton natif
                    // 2. Premier chargement → guide manuel "⋮ → Ajouter à l'écran"
                    const showAndroid = () => {
                        if (this.showBanner) return;
                        this.platform = pwaInstallPrompt ? 'android-native' : 'android-guide';
                        this.showBanner = true;
                    };

                    setTimeout(showAndroid, 4000);
                    window.addEventListener('pwa-installable', () => {
                        pwaInstallPrompt && (this.platform = 'android-native');
                        if (!this.showBanner) setTimeout(showAndroid, 1500);
                    });
                },
