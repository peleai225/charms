    ...
    <footer class="bg-gradient-to-b from-slate-900 to-slate-950 text-slate-300 mt-20">
        <!-- Newsletter -->
        <div class="border-b border-slate-800/80 relative overflow-hidden">
            {{-- Orbes subtiles --}}
            <div class="absolute -top-20 -left-20 w-80 h-80 bg-primary-600/10 rounded-full blur-3xl"></div>
            <div class="absolute -bottom-20 -right-20 w-96 h-96 bg-violet-600/10 rounded-full blur-3xl"></div>
            
            <div class="container mx-auto px-4 py-16 relative z-10">
                <div class="max-w-2xl mx-auto text-center">
                    <span class="inline-flex items-center gap-2 px-4 py-1.5 bg-primary-500/10 border border-primary-500/20 text-primary-400 text-xs font-semibold uppercase tracking-widest rounded-full mb-6 relative overflow-hidden">
                        <span class="absolute inset-0 bg-primary-500/20 w-1/4 -translate-x-full animate-[shimmer_2s_infinite]"></span>
                        Newsletter
                    </span>
                    <h3 class="text-3xl font-extrabold text-white mb-3 tracking-tight">Restez dans la boucle</h3>
                    ...
                    <form method="POST" action="{{ route('newsletter.subscribe') }}" class="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto no-ajax">
                        @csrf
                        <div class="relative flex-1">
                            ...
                            <input
                                ...
                                class="w-full pl-11 pr-5 py-4 bg-white/5 border border-white/10 rounded-2xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all backdrop-blur-sm shadow-inner"
                            >
                        </div>
                        <button type="submit" class="px-8 py-4 bg-gradient-to-r from-primary-600 to-primary-500 text-white font-bold rounded-2xl hover:-translate-y-0.5 transition-all shadow-lg hover:shadow-primary-600/30 whitespace-nowrap">
                            S'inscrire
                        </button>
                    </form>
                    <p class="text-slate-500 text-xs mt-4">Désabonnement possible à tout moment.</p>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        @if($socialFacebook)
                        <a href="{{ $socialFacebook }}" target="_blank" rel="noopener" class="w-10 h-10 bg-slate-800 rounded-full flex items-center justify-center text-slate-400 hover:bg-blue-600 hover:text-white transition-all" title="Facebook">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                        </a>
                        @endif
                        @if($socialInstagram)
                        <a href="{{ $socialInstagram }}" target="_blank" rel="noopener" class="w-10 h-10 bg-slate-800 rounded-full flex items-center justify-center text-slate-400 hover:bg-gradient-to-br hover:from-purple-600 hover:to-pink-500 hover:text-white transition-all" title="Instagram">
                        ...
                        @endif
                        @if($socialTwitter)
                        <a href="{{ $socialTwitter }}" target="_blank" rel="noopener" class="w-10 h-10 bg-slate-800 rounded-full flex items-center justify-center text-slate-400 hover:bg-sky-500 hover:text-white transition-all" title="Twitter / X">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/></svg>
                        </a>
                        @endif
                        @if($socialWhatsapp)
                        <a href="https://wa.me/{{ preg_replace('/\D/', '', $socialWhatsapp) }}" target="_blank" rel="noopener" class="w-10 h-10 bg-slate-800 rounded-full flex items-center justify-center text-slate-400 hover:bg-green-500 hover:text-white transition-all" title="WhatsApp">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                    <p class="text-sm text-slate-500">
                        ...
                        @if($footerText = \App\Models\Setting::get('footer_text'))
                            <span class="mx-1">&middot;</span> {{ $footerText }}
                        @endif
                    </p>
                    <div class="flex items-center gap-3">
                        <!-- Orange Money -->
